...
def test_direct_input():
    ...
    with patch.object(sys, "argv", ["interpreter", test_input]):
        args = parse_args()
        assert args["input"] == test_input  # Input is used directly now


@pytest.mark.asyncio
async def test_load_interpreter():
    # Test interpreter loading with custom settings
    # Create a mock profile with default settings
    mock_profile = Profile()
    
    # Create args that override some settings
    args = {
        "model": "gpt-4",  # Different from default
        "temperature": 0.7,  # Different from default
        "auto_run": True,   # May be same as default
        "debug": True       # May be same as default
    }
    
    # Mock the interpreter class to use our mock profile
    with patch('interpreter.interpreter.Profile', return_value=mock_profile):
        interpreter = load_interpreter(args)
        
        # Check that CLI args override profile settings
